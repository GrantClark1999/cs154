...
def adder(a, b, cin):
   ...
   for i in range(len(a)):
      sum[i], cout = half_adder(a[i], b[i], cin)
      cin = cout
   full_sum = concat_list([sum[i] for i in range(n)])
   return full_sum, cout

def mux2to1(input, switch, numBits):
   output = pyrtl.Output(bitwidth=numBits, name='output')
...
def mux4to1(input, s, numBits):
   output = pyrtl.Output(bitwidth=numBits, name='output')
   with pyrtl.conditional_assignment:
      with ~s[0]
         o |= mux2to1(input[0:2], s[1], numBits)
      with s[1]:
         o |= mux2to1(input[2:4], s[1], numBits)
   return output

def mux8to1(input, s, numBits):
   output = pyrtl.Output(bitwidth=numBits, name='output')
   with pyrtl.conditional_assignment:
      with ~s[0]
         o |= mux2to1(input[0:4], s[1:3], numBits)
      with s[1]:
         o |= mux2to1(input[4:8], s[1:3], numBits)
   return output
